...
exports.createContribution = async (req, res) => {
  try {
    ...
    if (!networkObj.success) {
      return res
        .status(500)
        .json({ success: false, message: networkObj.message });
    }
...
exports.getContribution = async (req, res) => {
  try {
    ...
    const { contract, gateway } = networkObj;
    const contributionBuffer = await contract.evaluateTransaction(
      'GetContribution',
      id
    );
...
exports.getAllContributions = async (req, res) => {
  try {
    ...
    const { contract, gateway } = networkObj;
    const contributionsBuffer = await contract.evaluateTransaction(
      'GetAllContributions'
    );
